...
- name: Wenjun Nora Liu
  affiliation: 
  - Jose Polo Group, Adelaide Centre for Epigenetics, School of Biomedicine, University of Adelaide
  - Dame Roma Mitchell Cancer Research Laboratories, Adelaide Medical School, University of Adelaide
...
pacakge: sSNAPPY
bibliography: '`r system.file("references.bib", package = "sSNAPPY")`'
output: 
...
While the method was originally developed for bulk-RNA studies, *sSNAPPY* can also applied to pseudo-bulked scRNA-seq dataset. 
...
## Compute weighted single-sample logFCs (ssFCs)
...
Before single-sample logFCs (ssFCs) can be computed, row names of the logCPM matrix need to be converted to EntrezGene IDs. This is because all the pathway topology information retrieved will be in EntrezGene IDs. The conversion can be achieved through bioconductor packages `AnnotationHub` and `ensembldb`.

To compute the ssFCs, samples must be in matching pairs. In our example data, treated samples were matched to the corresponding control samples derived from the same patients. Therefore the `groupBy` parameter of the `weight_ss_fc()` functions will be set to be `"patient"`. 
...
The output of the `weight_ss_fc()` function is a list with two element, where one is the weighted ssFCs matrix and the other is a vector of gene-wise weights.
...
This step is achieved through a chain of functions that are part of the `r Biocpkg("grapghite")`, which have been nested into one simple function in this package called `retrieve_topology()`. The `retrieve_topology` function now supports various species and databases. Databases that are currently supported for human are the Kyoto Encyclopaedia of Genes and Genomes (KEGG)[@OgataKEGGKyotoEncyclopediaa] database, the Reactome[@reactome2021] database, and WikiPathways[@Martens2021]. 

The downloaded topology matrices will be stored as a list where each element corresponds a pathway. It's recommended to save the list as a file so this step only needs to be performed once for each database.

This vignette chose KEGG pathways in human as an example.
...
For each round of permutation, sample labels are randomly shuffled to form permuted sample pairs while the remaining workflow remains unchanged.In a dataset with $N$ total samples, the maximum number of possible unique psuedo-pairs is $N \times (N-1)$.

The output of the `generate_permuted_scores()` function is a list where each element is a vector of permuted perturbation scores for a pathway. Unless otherwise specified, pathways with all zero permuted perturbation scores will be automatically dropped.
...
Permutation p-values of each indiviudal single-sample perturbation scores will be computed and corrected for multiple-testing using a user-defined approach. The default is `fdr`.
...
Nodes in the network plots could be colored by the pathways' predicted direction of perturbation (i.e.. sign of robust z-score):
...
```{r sigGS_nt_pvalue, fig.width= 10, fig.height=4,fig.cap="*Pathways significantly perturbed in individual samples, where gene-sets were colored by pathways' -log10(p-values).*"}
pl <- normalisedScores %>%
    dplyr::filter(adjPvalue < 0.05) %>%
    droplevels() %>%
    split(f = .$sample) %>%
