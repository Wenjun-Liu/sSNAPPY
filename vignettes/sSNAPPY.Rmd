...
pacakge: sSNAPPY
bibliography: '`r system.file("bibliography.bib", package = "sSNAPPY")`'
output: 
...
Pathways where all gene have zero gene-wise perturbation scores or all samples have zero pathway-level scores will be dropped by default. 
...
The output of the `generate_permuted_scores()` function is a list where each element is a vector of permuted perturbation scores for a specific pathway. Pathways with all zero permuted scores will be dropped by default. 
...
In the example data-set, the key question is how tumour tissues responded to the activation of PR alone or both ER and PR. We can test for the treatment-level pathway perturbation using a linear regression model of the form `~ 0 + treatment`.
...
```{r}
load(system.file("extdata", "gsAnnotation_df.rda", package = "sSNAPPY"))
```
