...
```{r permutedScore}
system.time(permutedScore <- generate_PermutedScore(
...
Let's randomly choose six pathways and examine the permutation-derived empirical distribution of their perturbation scores. They should all be approximately normal distributed. 

```{r NullDistribution, fig.width=8, fig.height=6, message=FALSE}
pl <- permutedScore %>%
    keep(~all(.!=0)) %>%
    .[sample(seq_along(.), 6)] %>%
    lapply(
