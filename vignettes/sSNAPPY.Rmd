...
This vignette demonstrates how to use the package `sSNAPPY` to compute directional single sample pathway perturbation scores by incorporating pathway topologies, utilizing sample permutation to test the significance of individual scores and comparing average pathway activities across treatments.
...
The `weight_ss_fc()` function firstly computes raw ssLogFCs for each gene by subtracting logCPM values of the control sample from the logCPM values of treated samples within each patient.

It has been demonstrated previously that in RNA-seq data, lowly expressed genes turn to have larger variances, which is also demonstrated in the plot below. To reduce the impact of this artifact, `weight_ss_fc` also weights each ssLogFCs by estimating the mean-variance relationship of logCPM, and defining the gene-wise weight to be the inverse of the predicted variance of that gene's mean logCPM value.
...
The output of the `generate_permuted_scores()` function is a list where each element is a vector of permuted perturbation scores for one specific pathway.
...
Two-sided p-values associated with each perturbation scores are also computed by the proportion of permuted scores that are more extreme than the test perturbation score within each pathway. Raw p-values will be corrected for multiple-testing using a user-defined approach. The default is `fdr`.

In a data with N samples, the total number of possible permuted pairs of samples is $N \times (N-1)$. When the sample size is small, small p-values cannot be accurately estimated so the p-values returned by the `normalise_by_permu()` function should be intepreted with caution.
...
It is not surprising to see that the estrogen signalling pathway was significantly activated under both R5020 (progesterone) and E2 (estrogen) +R5020 treatments.
...
```{r fig.height= 7, fig.width=8, fig.cap="*Gene-level perturbation scores of genes with top 15 highest absolute mean gene-wise perturbation scores in the Ovarian steroidogenesis gene-set.*" }
...
Since `plot_gene_contribution()` is built on `pheatmap`, which provides a practical column annotation feature, the `plot_gene_contribution()` function also allow a `data.frame` storing annotation information to be provided. We can annotate each column (ie. each sample) by the direction of pathway perturbation in that sample or any other sample metadata, such as their treatments. 

In this example, we first create a `data.frame` storing the pathway perturbation scores of the "Ovarian steroidogenesis" pathway in each sample and their treatments.
...
```{r fig.height= 7, fig.width=8, fig.cap="*Gene-level perturbation scores of genes with top 15 absolute mean gene-wise perturbation scores in the Ovarian steroidogenesis gene-set.Genes playing the most important roles in pathway activation was INS, CGA, GNAS.*"}
...
By default, `plot_gs_network()` function does not include nodes that are not connected to any other nodes, which could be overwritten by setting the `plotIsolated` parameter to TURE.
...
```
`sSNAPPY` was built in with categorisations of *KEGG* pathways. But to annotate pathways retrieved from other databases, customized annotation `data.frame` could be provided through the `gsAnnotation` parameter. 
