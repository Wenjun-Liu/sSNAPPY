...
In a data with N samples, the total number of possible permuted pairs of samples is $N \times (N-1)$. When the sample size is small, the p-values derived from the robust z-scores should be interpreted with caution. The MAD and median derived from the permuted scores are more likely to be accurately estimated when the number of permuted scores computed for each pathway is above 100, meaning that at least 11 samples are required. 
...
```{r fig.height= 5, fig.width=8, fig.cap="*Gene-level perturbation scores of genes with top 10 highest absolute mean gene-wise perturbation scores in the Ovarian steroidogenesis gene-set.*" }
plot_gene_contribution(
    genePertMatr  = genePertScore$`kegg.Ovarian steroidogenesis`,
    # only plot genes with top 10 highest absolute mean gene-wise scores
    filterBy = "mean"
)
```
...
We can also annotate each column (ie. each sample) by the direction of pathway perturbation in that sample or any other sample metadata, such as their treatments. To annotate the columns, we need to create an annotation `data.frame` with desired attributes. 
