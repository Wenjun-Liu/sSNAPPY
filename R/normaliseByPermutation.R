...
#'
#' @details
#' This \code{generatePermutedScores} function is a generic function that can deal with
...
#' permutedScore <- generatePermutedScores(logCPM_example, numOfTreat = 3,
...
#'  }
#' @export
setGeneric("generatePermutedScores",
           function(expreMatrix, numOfTreat,NB = 1000,
                    gsTopology, weight, BPPARAM = BiocParallel:: bpparam())
               standardGeneric("generatePermutedScores"))

#' @rdname generatePermutedScores
setMethod("generatePermutedScores",
...
setMethod("generatePermutedScores",
          ...
          function(expreMatrix, numOfTreat,NB = 1000,
                   gsTopology, weight, BPPARAM = BiocParallel:: bpparam()){
              generatePermutedScores(as.matrix(expreMatrix), numOfTreat,NB,
                                     gsTopology, weight, BPPARAM = BiocParallel:: bpparam())
          })
...
setMethod("generatePermutedScores",
          ...
          function(expreMatrix, numOfTreat,NB = 1000,
              ...
              generatePermutedScores(expreMatrix, numOfTreat,NB,
                                     gsTopology, weight, BPPARAM = BiocParallel:: bpparam())
          })
...
#' the `generatePermutedScores` function. The mean absolute deviation(MAD) and median of perturbation scores for each pathway are firstly
...
#' calculated.
#' @param permutedScore A list. Output of `generatePermutedScores`
...
#' permutedScore <- generatePermutedScores(logCPM, numOfTreat = 2,
#'  NB = 5, gsTopology = gsTopology, weight = ls$weight)
#' normalisedScores <- normaliseByPermu(permutedScore, ssPertScore)
#'  }
#'  # Load the example output
#'  load(system.file("extdata", "normalisedScores.rda", package = "sSNAPPY"))
#'  normalisedScores
normaliseByPermu <- function(permutedScore, testScore, pAdj_method = "fdr"){
    pvalue <- NULL
